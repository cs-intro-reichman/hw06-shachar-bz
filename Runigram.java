...
public class Runigram {
	...
	public static Color[][] grayScaled(Color[][] image) {
		...
		for(int i = 0; i < greyImage.length; i++){
			for(int j = 0; j < greyImage[i].length; j++){
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for(int i = 0; i < scaledImage.length; i++){	
			for(int j = 0; j < scaledImage[i].length; j++){
				...
				scaledImage[i][j] = image[(int) (i * newHeight)][(int) (j * newWidth)];
			}
		}
		return scaledImage;
	}
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		Color blendTwo = new Color(red, green, blue);
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		// check if the target image have the same dimensions as the source image
		// if not, scale it
		...
		// Morphs the source image into the target image
		for(int i = 0; i <= n; i++){
